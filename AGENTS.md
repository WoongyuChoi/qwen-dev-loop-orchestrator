...
- 프로젝트 디렉터리 모드는 더블클릭 2번 진입 시 첫 요청에서 `next_question.txt`와 이전 `last_turn.txt`를 우선하지 않고 fresh scan seed를 사용하며, 스캔된 실제 디렉터리/파일 역할에서 동적 diversity group을 만든 뒤 그룹을 먼저 샘플링하고 그 안의 대표 파일을 primary question target으로 뽑는지 확인한다. `project_scan_summary.*`에는 `Diversity Groups`, primary group, 후보별 group이 남아야 하며, `src/api`처럼 파일 수와 score가 높은 단일 폴더가 첫 질문 후보를 독점하지 않아야 한다. 코드 후보가 있으면 `.md` 문서는 primary target에서 제외하되 scan context 참고용으로는 남길 수 있다. 첫 질문은 업무 도메인/사용자 시나리오, 정상 데이터·상태 흐름, 연결 파일 확인 이유를 먼저 묻고 예외/성능/보안은 마지막 리스크 점검 항목으로 다루게 해야 한다. 기존 `qwen-loop-data` 최근 질문은 중복 회피용 히스토리로 prompt에 포함된다. 직접 `qwen-loop.ps1 -ProjectRoot`를 `-FreshProjectQuestion` 없이 호출하는 고급 경로에서는 기존 continuation 동작을 유지한다.
